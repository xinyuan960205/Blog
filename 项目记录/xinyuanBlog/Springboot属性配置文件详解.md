...
```
com.didispace.blog.name=xinyuan
...
public class ApplicationTests {
	...
	public void getHello() throws Exception {
		Assert.assertEquals(blogProperties.getName(), "xinyuan");
...
我们启动工程输入[http://localhost:8080](http://localhost:8080/) 就可以看到打印了。

有时候属性太多了，一个个绑定到属性字段上太累，官方提倡绑定一个对象的bean，这里我们建一个ConfigBean.java类，顶部需要使用注解@ConfigurationProperties(prefix = “com.didispace.blog”)来指明使用哪个
...
`com.didispace.blog.desc`参数引用了上文中定义的`name`和`title`属性，最后该属性的值就是`xinyuan正在努力写《Spring Boot教程》`。
...
[![img](http://image.tengj.top/springboot2-1.jpg)](http://image.tengj.top/springboot2-1.jpg)
